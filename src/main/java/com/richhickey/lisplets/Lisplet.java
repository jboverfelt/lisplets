...
public class Lisplet extends HttpServlet {
    ...
    protected void sendUser(HttpServletRequest req, Writer writer)
        ...
        if (roles.length > 0) {
            ...
            for (String role : roles) {
                writeTagLiteralPair(writer, role, boolSym(req.isUserInRole(role)));
            }
            writer.write(")");
        }
        writer.write(")");
    ...
    protected void sendServlet(Writer writer)
            throws IOException {
        writer.write("(");
    ...
    protected void sendContext(Writer writer)
            throws IOException {
        ServletContext context = getServletContext();
        writer.write("(");
    ...
    protected void sendParameters(HttpServletRequest req, Writer writer)
        ...
        for (Map.Entry<String, String[]> stringEntry : req.getParameterMap().entrySet()) {
            ...
            for (String val : vals) {
                writer.write(" ");
                writeReadableString(writer, val);
            }
            writer.write(")");
        }
        writer.write(")");
